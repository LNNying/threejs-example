function Three_ShapeDrawer(options){
  ...
  var texture1 = THREE.ImageUtils.loadTexture("pic/muwen.jpg");
  mat.map=texture1;
  this.material = mat;
  ...
  function init() {
    ...
    //th.scene.fog = new THREE.Fog( 0xffffff, 10, 1000 );
	...
	//th.light = new THREE.SpotLight( 0x444444 );
    ...
    renderer.setClearColor( 0xffffff, 1 );//0xbfd1e5
...
Three_ShapeDrawer.prototype.add = function(body,shape,options){
  ...
  if(options && options.threemesh){
    ...
    case DemoApplication.prototype.BOX_SHAPE_PROXYTYPE:
	  ...
	  {
		  var mat = new THREE.MeshPhongMaterial();
	  var texture1 = THREE.ImageUtils.loadTexture("pic/box.bmp");
	  mat.map=texture1;
      mesh = new THREE.Mesh (new THREE.CubeGeometry( h.x()*2, h.y()*2, h.z()*2 ),
		  ...
		  if(h.x()*2>10){
			  var mat = new THREE.MeshPhongMaterial();
	  var texture1 = THREE.ImageUtils.loadTexture("pic/grass.png");
