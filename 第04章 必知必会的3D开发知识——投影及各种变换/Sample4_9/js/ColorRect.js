function ColorRect(								//声明绘制用物体对象所属类
	gl,						 					//GL上下文
	programIn	//着色器程序id
	){
	this.vertexData=[
	...
	{	
		...
		//获取位置、旋转变换矩阵引用id
        var muMMatrixHandle = gl.getUniformLocation(this.program, "uMMatrix");
		//将位置、旋转变换矩阵传入shader程序
        gl.uniformMatrix4fv(muMMatrixHandle,false,new Float32Array(ms.getMMatrix())); 
		...
		gl.drawArrays(gl.TRIANGLE_FAN, 0,this.vcount); 
	}
}
