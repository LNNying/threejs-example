function Ball(								//声明绘制用物体对象所属类
	...
	this.initVertexData=function(){
		...
		for(var vAngle = -90; vAngle < 90; vAngle = vAngle + angleSpan)//-90  90
		{
			for (var hAngle = 0; hAngle < 360; hAngle = hAngle + angleSpan)// 水平方向angleSpan度一份0  360
				{// 纵向横向各到一个角度后计算对应的此点在球面上的坐标
					...
					this.vertexData.push(x1,y1,z1);//1
					this.vertexData.push(x3,y3,z3);//3
					this.vertexData.push(x0,y0,z0);//0
					
					this.vertexData.push(x1,y1,z1);//1
					this.vertexData.push(x2,y2,z2);//2
					this.vertexData.push(x3,y3,z3);//3
				}
		}
	};
